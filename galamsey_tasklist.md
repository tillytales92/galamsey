...
- [ ] **NDVI/EVI measurement choice outstanding — rebuild as a "peak EVI" pipeline (Vashold et al.
  2026 methodology).** *(PARTIAL 2026-07-03: step 1 done — the MODIS VI source in `d_01` Sec 5c was
  switched from `MOD13A2` (1 km) to `MOD13Q1` (250 m); Landsat NDVI/EVI export also moved 250 m → 30 m.
  Steps 3–5 below — ESA CCI mask, per-16-day zonal mean, annual-max "peak" reduction — remain to do.)*
  The current pipeline exports an **annual mean** of MODIS 16-day composites
  at the pixel level (GEE-side `.mean()`, `d_01_download_gee.R` Sec 5c, now product **MOD13Q1**, 250 m),
