...
- [~] Replace the 25 km centroid-block SE-clustering stand-in with a real sub-basin ID
  ...
  **Cluster count verified: 280 level-9 basins cover the 3,348-hex grid** (median 10 hex/basin, 18
  singletons; 42 coarser MAIN_BAS as fallback) — ample for clustered/CS SEs. **Remaining:** merge
  `hex_basin_5km.csv` into `event_panel_5km.rds` in `b_03e_assemble_eventpanel.R`, then switch the
  event-study cluster var from the 25 km centroid-block stand-in to `basin_num`
  (`a_05_event_study.Rmd` lines ~274–280 / `event_study_design.md` / `methodology_explainer.md`).
