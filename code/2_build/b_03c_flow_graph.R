...
# rather than routing over ridges/hillslopes. Routing traces on the NATIVE 4326 dir/upa
# grids — reprojecting D8 pointer codes corrupts routing (confirmed cell-level cycles on
# the UTM grid; see d_04_merit.R header).
#
# TWO treatment definitions are built, at two ROUTE_KM2 cuts (d_04_merit.R Sec 11j sweep):
#   10 km² (PRIMARY, unsuffixed files)  — reaches small tributaries; 5.5% of mined ha
#                                          off-network (never routed downstream).
#   50 km² (ALT, "_upa50" files)        — trunk-stream only, closer to the OSM natural-river
#                                          network (STREAM_KM2 in d_04_merit.R); 23% of mined
#                                          ha off-network at this coarser cut.
# Per-hex upstream-exposure *ranking* is stable across a {2,5,10,20} km² sweep (d_04 Sec 11j),
# so 50 km² is a genuinely different (coarser) network, not just noise around 10.
#
# Inputs:
...
#
# Outputs (data/processed/merit/ per resolution N x threshold suffix S in {"", "_upa50"}):
#   hex_flow_edges_{N}km{S}.csv      — tibble(from_hex, to_hex, n_crossings, flow_weight)
#   hex_downstreamness_{N}km{S}.csv  — tibble(hex_id, mean_log_upa, n_chan_cells)
#
...
ROUTE_CONFIGS <- list(       # ROUTE_KM2 cuts to build, each with its own output-file suffix
  list(route_km2 = 10, suffix = ""),        # primary — unsuffixed, matches existing cached files
  list(route_km2 = 50, suffix = "_upa50")   # alternative treatment definition
)
...
for (cfg in ROUTE_CONFIGS) {
  message(sprintf("  Channel cells at ROUTE_KM2 = %g km²: %d",
                  cfg$route_km2, sum(terra::values(upa_r) > cfg$route_km2, na.rm = TRUE)))
}
...
# Pull to matrices once (wide = row-major; persistent across the resolution/threshold loops)
dir_m <- terra::as.matrix(dir_r, wide = TRUE)
upa_m <- terra::as.matrix(upa_r, wide = TRUE)
nr    <- nrow(dir_m)
nc    <- ncol(dir_m)

# Channel source cells at a given ROUTE_KM2: upa above threshold with a defined direction,
# plus their D8 target offsets. Returns everything the per-resolution edge build needs.
build_channels <- function(route_km2) {
  ...
  list(chan_idx = chan_idx, rr = rr, cc = cc, r2 = r2, c2 = c2,
       in_bounds = in_bounds, flow_w = upa_m[cbind(rr, cc)])
}

####2. Per-resolution loop (outer), per-threshold loop (inner) ####
...
for (res_km in have_res) {
  ...
  ####2a. Rasterise hex IDs onto the native 4326 MERIT grid (once per resolution) ####
  ...
  for (cfg in ROUTE_CONFIGS) {
    out_edges <- file.path(proc_dir, sprintf("hex_flow_edges_%dkm%s.csv",     res_km, cfg$suffix))
    out_down  <- file.path(proc_dir, sprintf("hex_downstreamness_%dkm%s.csv", res_km, cfg$suffix))
    message(sprintf("  --- ROUTE_KM2 = %g km²%s ---", cfg$route_km2,
                    if (nzchar(cfg$suffix)) sprintf(" (%s)", cfg$suffix) else " (primary)"))

    chan <- build_channels(cfg$route_km2)

    ####2b. Cross-hex edges from D8 cell pairs ####
    from_hex          <- hex_m[cbind(chan$rr, chan$cc)]
    to_hex            <- rep(NA_integer_, length(chan$rr))
    to_hex[chan$in_bounds] <- hex_m[cbind(chan$r2[chan$in_bounds], chan$c2[chan$in_bounds])]

    edge_ok   <- chan$in_bounds & !is.na(from_hex) & !is.na(to_hex) & from_hex != to_hex
    edges_agg <- tibble(from = from_hex[edge_ok],
                        to   = to_hex[edge_ok],
                        flow = chan$flow_w[edge_ok]) |>
      group_by(from, to) |>
    ...
    downstreamness_r <- tibble(hex_num = hex_m[chan$chan_idx],
                                upa     = upa_m[chan$chan_idx]) |>
    ...
    rm(chan, edges_agg, edges_net, g_r, downstreamness_r, cmp_r)
  }

  rm(hex_sf_r, hex_ll, hex_id_r, hex_m, hex_north_r)
  gc()
}
